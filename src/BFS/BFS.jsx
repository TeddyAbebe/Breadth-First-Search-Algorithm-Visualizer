...
export default class BFS extends React.Component {
  ...
  componentWillMount() {
    ...
    this.setState({
      // set canvasSize and HexParamters in the state
  ...
  shouldComponentUpdate(nextProps, nextState) {
    // updates canvas bases on changes in state as well as controls update flow
    ...
    if (nextState.cameFrom !== this.state.cameFrom) {
      // if came from value in nextstate is different from current state, redraw canvas view to show updated arrows
      const { canvasWidth, canvasHeight } = this.state.canvasSize;
      const ctx = this.canvasView.getContext("2d");
      ctx.clearRect(0, 0, canvasWidth, canvasHeight);
      for (let l in nextState.cameFrom) {
        // loops throgh each item in came from to draw arrows between hexes
  ...
  getHexCornerCoord(center, i) {
    // gets coords of hexagons corners, returns as a point, draws a line between them
  ...
  drawHex(canvasID, center, lineWidth, lineColor, fillColor) {
    // draws a hex on the canvas with provided parameters
    for (let i = 0; i <= 5; i++) {
      // loops through all 6 corners of hex
  ...
  drawLine(canvasID, start, end, lineWidth, lineColor) {
    // draws a line on the canvas between the start and end points
  ...
  drawHexes() {
    // draws hexagonal grid on canvas based on the provided canvas size and hexParameters
    const { canvasWidth, canvasHeight } = this.state.canvasSize;
    ...
    for (let r = 0; r <= rBottomSide; r++) {
      if (r % 2 === 0 && r !== 0) {
        // adjusts horizontal position based on row value
        p++;
      }
      for (let q = -qLeftSide; q <= qRightSide; q++) {
        ...
        if (
          ...
          // check if hexes center lies within a visible area of canvas
          ...
          if (!this.state.obstacles.includes(bottomH)) {
            // if hex is not marked by obstacle adds to hex path map arr
    ...
    for (let r = -1; r >= -rTopSide; r--) {
      // loops through hexes in visible top size
      if (r % 2 !== 0) {
        n++;
      }
      for (let q = -qLeftSide; q <= qRightSide; q++) {
  ...
  hexToPixel(h) {
    // converts hex coords to pixel coords, returns an x,y point
  ...
  drawHexCoordinates(canvasID, center, h) {
    // for each hex draws col and row from center value
    const ctx = canvasID.getContext("2d");
  ...
  getHexParameters() {
    // calculates hex parameters based on the hexSize and return as an object
  ...
  handleMouseMove(e) {
    // handle mouse move event to update the current hex and its details
    ...
    ); // calculates cube coords of hex under mouse cursor
    ...
    if (
      ...
      // checls if hexes center lies within visible area or not
      this.setState({
        // updates current hex state to store cube and pixel coords under mouse cursor
  ...
  getCanvasPosition(canvasID) {
    ...
    this.setState({
      canvasPosition: {
        ...
        top: rect.top,
        bottom: rect.bottom,
      }, // setting current state to where the canvas is, always get correct canvas position
    });
  }
  ...
  cubeRound(cube) {
    // rounds cube coords to integral hex coordinates returns hex
    ...
    if (x_diff > y_diff && x_diff > z_diff) {
      // determines axis and adjust coord to maintain sum of q,r,s as 0
      rx = -ry - rz;
  ...
  cubeDirections(direction) {
    // gets cube coords for the hex's neighboring directions
  ...
  cubeAdd(a, b) {
    // returns addition of all hexagons up until mouse cursor
    return this.Hex(a.q + b.q, a.r + b.r, a.s + b.s);
  }

  cubeSubtract(hexA, hexB) {
    // returns as a new hex after subtracting one cube coord from another
    return this.Hex(hexA.q - hexB.q, hexA.r - hexB.r, hexA.s - hexB.s);
  }

  getCubeNeighbor(h, direction) {
    // gets neigboring cube coord in the specified direction from any given hex
  ...
  linearInt(a, b, t) {
    // linear interpolation between two hexes in cuve coords
    return a + (b - a) * t; // returns new hex
  }

  cubeLinearInt(hexA, hexB, t) {
    // linear interpolation between two hexes in cube coords and return as a new hex
    return this.Hex(
  ...
  getDistanceLine(hexA, hexB) {
    // gets distance line between two hexes
  ...
  handleClick() {
    // handle click event on canvas, updates users position
    const { currentHex, cameFrom } = this.state;
    const { q, r, s } = currentHex;
    console.log(q, r, s);
    if (cameFrom[JSON.stringify(this.Hex(q, r, s))]) {
      // check if clicked hex is reachable from users current position
      this.setState(
        // update users current position, triggers bfs method call
  ...
  getNeighbors(h) {
    // gets neighboring hexagons for a given hexagon and return as arr
    var arr = [];
    for (let i = 0; i <= 5; i++) {
      const { q, r, s } = this.getCubeNeighbor(this.Hex(h.q, h.r, h.s), i);
  ...
  breadthFirstSearch(playerPosition) {
    ...
    while (frontier.length !== 0) {
      ...
      for (let l of arr) {
        // Use regular for loop instead of map to fix the no-loop-func warning
        if (
  ...
  drawPath() {
    // draw the path on the canvas based on calculated path
  ...
  drawArrow(fromx, fromy, tox, toy) {
    // draws an arrow from point to another
